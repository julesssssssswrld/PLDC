...
:: Handles: Python check, dependencies, firewall config, IP config, and server start
...
:: Step 4: Configure IP Address (192.168.1.200)
echo   [4/5] Configuring IP Address (192.168.1.200)...
...
:: Remove legacy IP 192.168.1.2 if present
powershell -ExecutionPolicy Bypass -NoProfile -Command "try { Remove-NetIPAddress -IPAddress '192.168.1.2' -Confirm:$false -ErrorAction SilentlyContinue } catch { }" >nul 2>&1

:: Add 192.168.1.200
ipconfig | findstr /C:"192.168.1.200" >nul 2>&1
if !errorLevel! equ 0 (
    echo         [OK] IP 192.168.1.200 already configured
) else (
    netsh interface ipv4 add address name="!IFACE_NAME!" addr=192.168.1.200 mask=255.255.255.0 >nul 2>&1
    if !errorLevel! equ 0 (
        echo         [OK] IP 192.168.1.200 configured on !IFACE_NAME!
    ) else (
        echo         [WARNING] Failed to add IP 192.168.1.200. Please check network settings.
    )
)
...
echo     - Admin URL:    http://192.168.1.200
